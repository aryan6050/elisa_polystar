...
def create_index_pattern(config):
    try:
        ...
        for index, row in excel_data.iterrows():
            ...
            print("the url pattern is ",url_pattern)
            headers = {'Content-Type': 'application/json'}
            response_pattern = requests.put(url_pattern, auth=(username, password), headers=headers, json=index_pattern)
                
            if response_pattern.status_code == 200 or response_pattern.status_code == 201:
                logging.info(f"Index pattern - {name} created successfully!")
            else:
                logging.error(f"Failed to create index pattern - {name}. Status code: {response_pattern.status_code}")
